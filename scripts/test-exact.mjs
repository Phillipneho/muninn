...
async function test() {
  ...
  const predicate = 'qa_supports'; // What PREDICATE_MAP returns
  
  console.log(`Testing: "${predicate}" -> "${expected}"`);
  
  // Step 1: Try plain predicate (strips qa_)
  const plainPredicate = predicate.startsWith('qa_') ? predicate.replace('qa_', '') : predicate;
  console.log(`Plain predicate: ${plainPredicate}`);

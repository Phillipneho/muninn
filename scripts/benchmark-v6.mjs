#!/usr/bin/env node
/**
 * LOCOMO Benchmark v6 - Use predicate parameter correctly
 */
...
async function searchFacts(entity, predicate) {
  ...
  params.set('predicate', predicate);  // This is the key fix
  params.set('limit', '5');
  ...
  // Return the first result that matches our predicate
  for (const fact of data.results) {
    if (fact.predicate === predicate) return fact;
  }
...
async function runBenchmark() {
  console.log('=== LOCOMO Benchmark v6 ===\n');
  console.log('Testing: Q&A predicates with proper API usage\n');
  ...
  let samples = { temporal: [], identity: [], relationship: [], other: [] };
  ...
  for (const conv of locomo) {
    ...
    for (const qa of conv.qa) {
      ...
      // Collect samples
      if (samples[catName].length < 5) {
        samples[catName].push({
          question: q.substring(0, 60),
          expected: String(expected).substring(0, 40),
  ...
  for (const [cat, items] of Object.entries(samples)) {
    ...
    for (const item of items) {
      console.log(`  ${item.correct ? '✓' : '✗'} Q: ${item.question}...`);

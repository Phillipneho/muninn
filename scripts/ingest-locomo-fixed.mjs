#!/usr/bin/env node
/**
 * Fixed LOCOMO ingestion - Passes entities array to ensure proper entity resolution
 */
...
async function storeFacts(facts) {
  ...
  // Get unique entities from facts
  ...
  const res = await fetch(`${MUNINN_API}/facts`, {
    ...
    body: JSON.stringify({ facts, entities }) // IMPORTANT: Pass entities array
  });
...
async function main() {
  console.log('=== Fixed LOCOMO Ingestion ===\n');
  ...
  for (const conv of locomo) {
    ...
    console.log(`[${conv.sample_id}] Extracted ${facts.length} facts`);
